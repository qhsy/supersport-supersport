...
public class ApiUserExpertList extends RootApiBase<ApiUserExpertListInput, ApiUserExpertListResult> {
	...
	protected ApiUserExpertListResult process(ApiUserExpertListInput input) {
		...
		if (StringUtils.isNotBlank(input.getZoo().getToken())) {
			...
			String userCode = userCallFactory.upUserCodeByAuthToken(input.getZoo().getToken(),
	...
	public List<UcUserinfoExpert> initPageList() {
		...
		Page<UcUserinfoExpert> expertPage = serviceFactory.getUserInfoExpertService().queryPageByConditon(1, 10,
		...
		Page<UcUserinfoExpert> expertPowerPage = serviceFactory.getUserInfoExpertService().queryPageByConditon(1, 10,
				zeroCondition, zeroSort);
