...
import com.uhutu.dcom.user.z.tecent.entity.AccountImport;
...
public class ApiForLogin extends RootApiBase<ApiForLoginInput, ApiForLoginResult> {
	...
	public ApiForLoginResult process(ApiForLoginInput inputParam) {
		...
		if (StringUtils.isNotBlank(result.getUserCode())) {
			...
			UcUserinfoExt ext = JdbcHelper.queryOne(UcUserinfoExt.class, "userCode", ucUserinfo.getCode());
			...
			base.postTecent(ConfigDcomUser.upConfig().getLoginSdkAccountImport(), accountImport);
			...
			textElem.setText("您家水表已被查,您的登录手机号为：" + ucUserinfo.getLoginName() + "，用户编号为：" + ucUserinfo.getCode());
