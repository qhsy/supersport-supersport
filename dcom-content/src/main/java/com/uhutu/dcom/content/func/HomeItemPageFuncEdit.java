...
public class HomeItemPageFuncEdit extends RootFunc {
	...
	public WebOperateResult process(WebPageModel webPageModel, ExtendPageDefine extendPageDefine,
		...
		if ("dzsd4107100110110001".equals(input.getDataMap().get("type"))) {
			...
			List<CnHomeItem> li = JdbcHelper.queryForList(CnHomeItem.class, "", "",
					" za!=:za and type='dzsd4107100110110001' and ((endTime>=:startTime and endTime<=:endTime) or (startTime>=:startTime and startTime<=:endTime) or (startTime<=:startTime and endTime>=:endTime) or (startTime>=:startTime and endTime<=:endTime)) ",
					map);
