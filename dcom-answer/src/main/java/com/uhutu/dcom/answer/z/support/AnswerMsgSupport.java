...
public class AnswerMsgSupport {
	...
	public WechatMsgResponse sendAskMsg(AwQuestionInfo questionInfo,String requestUrl) {
		...
		/*系统push*/
		answerServiceFactory.getQuestionInfoService().saveAnswerMsg(title,content+"："+questionInfo.getContent(), questionInfo.getAnswerUserCode());
	 ...
	 * 		内容
	 */
	public void baiduPush(String userCode,String title, String content) {
		...
		try {
			...
			if (ucClientInfo != null && StringUtils.isNotBlank(ucClientInfo.getChannelId())) {
				new BaiduPush().push(PushEnum.ios, title, content, ucClientInfo.getChannelId());
			}
			...
			if (android != null && StringUtils.isNotBlank(android.getChannelId())) {
				new BaiduPush().push(PushEnum.android, title, content, android.getChannelId());
			}
