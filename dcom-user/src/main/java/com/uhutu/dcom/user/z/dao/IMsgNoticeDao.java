...
public interface IMsgNoticeDao extends JpaRepository<UcMsgNotice, String>,JpaSpecificationExecutor<UcMsgNotice> {
	...
	@Query(value = "select t.* from uc_msg_notice t where not exist(select 1 from uc_msg_notice_user p where p.user_code =:userCode and p.notice_code=t.code)",nativeQuery=true)
